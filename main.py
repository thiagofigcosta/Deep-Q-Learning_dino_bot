    ...
    while(ingame):
        ...
        for character,sprites in killer_instinct_cutted_sprites.items():
            if character in characters_in_game:
                ...
                candidate_spr=None # TODO remove
                for i in range(len(sprites)):
                    sprite=sprites[i]
                    sprite_w,sprite_h=sprite.shape[1],sprite.shape[0]
                    method=cv2.TM_SQDIFF # TM_SQDIFF or TM_CCORR_NORMED or TM_CCOEFF_NORMED
                    ...
                    if method in [cv2.TM_SQDIFF, cv2.TM_SQDIFF_NORMED]:
                        ...
                        loc=max_loc
                        val=max_val
                    if val>=img_match_threshold:
                        if candidate is None or val>candidate_val:
                            rec=spriter.pointAndSizeToRectangle(loc[0],loc[1],sprite_w,sprite_h)
                            candidate=rec
                            candidate_val=val
                            candidate_index=i
                            candidate_spr=sprite
                                
                if candidate is not None:
                    found_elements.append({'el':character,'rec':candidate,'idx':candidate_index,'spr':candidate_spr})
        ...
        for el in found_elements:
            ...
            cv2.imshow(el['el']+str(el['idx']),el['spr'])
